...
`.github/workflows/radar-health.yml` prueft die Produktions-URL viermal pro Stunde. Geprueft werden mindestens Version 5.2.1, Nur-Lese-Modus, Telegram-Konfiguration, alle persistenten Zustandsdateien, der Alpaca-IEX-Marktfeed, die konfigurierte und authentifizierte Oel-Datenquelle, alle Pflichtquellen samt Aktualitaet, Kimis strikte Shadow-Isolation sowie der fail-closed Trump-Archivvertrag (`trump.fm-public-api`, vollstaendige ID-/Zeit-/Checksum-Pruefung, unabhaengige Bestaetigung erforderlich, keine direkten Telegram-Alarme). Bei einem Fehler wird genau ein offenes GitHub-Issue angelegt und nach Erholung automatisch geschlossen.
