...
replaceRequired(
  "catalyst module import",
  'const path = require("path");',
  'const path = require("path");\nconst createCatalystEngine = require("./catalyst-engine");'
);
...
const runtimeControl = `function automaticScanWindowOpen() {
...
const catalystEngine = createCatalystEngine({
  sendMessage: sendTelegramMessage,
  telegramConfigured,
  marketWindowOpen: automaticScanWindowOpen,
  onAlert: (iso) => {
    lastAlertAt = iso;
  }
});
...
replaceRequired(
  ...
  runtimeControl + "async function runAutomaticRadar() {\n  if (!automaticScanWindowOpen()) return;"
);

replaceRequired(
  "catalyst status",
  "      lastAlertAt,\n      monitoring:",
  "      lastAlertAt,\n      catalystLastScanAt: catalystEngine.getState().lastScanAt,\n      catalystSources: catalystEngine.getState().sources,\n      monitoring:"
);

const catalystRoute = `  if (requestUrl.pathname === "/api/catalysts") {
    return sendJson(res, 200, catalystEngine.getState());
  }
